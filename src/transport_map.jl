...
GetParams(umap::TransportMap)::Vector{Float64} =
    __notImplement(GetParams, typeof(umap), TransportMap)
NumParams(umap::TransportMap)::Int = __notImplement(NumParams, typeof(umap), TransportMap)
SetParams(umap::TransportMap, ::__VR)::Nothing =
    __notImplement(SetParams, typeof(umap), TransportMap)
...
function LogDeterminantInputGrad(umap::TransportMap, pts::Vector{Float64})
    igrad, ihess = EvaluateMap(
        umap, pts, [DerivativeFlags.InputGrad, DerivativeFlags.InputHess]
    )
    ihess ./ igrad
end

function LogDeterminantParamGrad(umap::TransportMap, pts::Vector{Float64})
    igrad, mgrad = EvaluateMap(
        umap, pts, [DerivativeFlags.InputGrad, DerivativeFlags.MixedGrad]
    )
    mgrad ./ igrad'
end
