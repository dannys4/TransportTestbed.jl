...
@enum __DerivativeFlags ErrorFlag = 0 None = 1 ParamGrad = 2 InputGrad = 3 MixedGrad = 4 MixedHess =
    5 InputHess = 6
end
...
struct SigmoidParam{T<:SigmoidType,U<:TailType} <: MapParam
    ...
    function SigmoidParam{T1,U1}(
        ...
        if !(length(centers) == length(widths) && length(centers) == length(weights))
            throw(
                ArgumentError("centers, widths, and weights must all have the same length")
            )
        end
        for j in eachindex(centers)
            for vec in (centers[j], widths[j], weights[j])
                if length(vec) != j
                    throw(
                        ArgumentError(
                            "The jth argument of any nonempty input should be of length j"
                        ),
                    )
                end
            end
...
struct QuadPair{Q1,Q2} <: QuadRule where {Q1<:QuadRule,Q2<:QuadRule}
    ...
    function QuadPair(
        quad1::_Q1, quad2::_Q2; quad1_weight::Real=0.5, quad2_weight::Real=0.5
    ) where {_Q1<:QuadRule,_Q2<:QuadRule}
        total_weight = quad1_weight + quad2_weight
        if quad1_weight < 0 || quad2_weight < 0 || total_weight <= 0
            throw(
                ArgumentError(
                    "Must have nonnegative weights with nonzero sum. Given weights $quad1_weight, $quad2_weight",
                ),
            )
        end
...
struct CombinedLoss{P,PReg,SReg} <:
    ...
    function CombinedLoss(
        ...
        weight_sobolev_reg=0.0,
    ) where {P1,PReg1,SReg1}
        new{P1,PReg1,SReg1}(
            weight_primary,
            primary,
            weight_param_reg,
