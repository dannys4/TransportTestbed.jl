function TestSigmoidParamEvaluation()
    ...
    support_bound = 100.0
    xgrid = vcat([-support_bound, 0.0, support_bound], xgrid)
    f = CreateSigmoidParam(; centers)
    evaluations = EvaluateAll(f, 5, xgrid)
    @test all(evaluations[1, :] .== 1.0)
    ...
    @test all(diff_first_eval .>= 0.0) # Check if monotone
end
